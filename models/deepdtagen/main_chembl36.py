...
def train(model, device, train_loader, optimizer, mse_f, epoch, train_data, FLAGS):
    ...
    with tqdm(train_loader, desc=f"Epoch {epoch + 1}") as t:
        for i, data in enumerate(t):
            ...
            mse_loss = mse_f(Pridection, target)
...
def experiment(FLAGS, dataset, device):
    ...
    # Load tokenizer
    with open(os.path.join(base_dir, 'data', f'{dataset}_tokenizer.pkl'), 'rb') as f:
        tokenizer = pickle.load(f)

    # Load processed data
    processed_data_file_train = os.path.join(base_dir, 'data', 'processed', f'{dataset}_train.pt')
    processed_data_file_test = os.path.join(base_dir, 'data', 'processed', f'{dataset}_test.pt')
    if not (os.path.isfile(processed_data_file_train) and os.path.isfile(processed_data_file_test)):
        print("Please run create_data.py to prepare data in PyTorch format!")
    else:
        train_data = TestbedDataset(root=os.path.join(base_dir, 'data'), dataset=f'{dataset}_train')
        test_data = TestbedDataset(root=os.path.join(base_dir, 'data'), dataset=f'{dataset}_test')
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if (epoch + 1) % 20 == 0:
                ...
                filename = os.path.join(base_dir, 'weights', f'deepdtagen_model_{dataset}.pth')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train DeepDTAGen model')
    parser.add_argument('--dataset', type=str, default='chembl36_aug_reg', help='Dataset name')
