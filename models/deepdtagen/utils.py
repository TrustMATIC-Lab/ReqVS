...
class TestbedDatasetBatch(InMemoryDataset):
    """
    分批处理的TestbedDataset，用于处理大数据集，避免内存溢出
    保留原有TestbedDataset的所有功能，但使用分批处理方式
    """
    def __init__(self, root='/tmp', dataset='davis', 
                 xd=None, xdt=None, xt=None, y=None, transform=None,
                 pre_transform=None, smile_graph=None, batch_size=50000):
        """
        Args:
            batch_size: 每批处理的数据量，默认50000，可根据内存调整
        """
        super(TestbedDatasetBatch, self).__init__(root, transform, pre_transform)
        self.dataset = dataset
        self.pad_token = Tokenizer.SPECIAL_TOKENS.index('<pad>')
    ...
    def process(self, xd, xdt, xt, y, smile_graph):
        ...
        print(f'Processing {data_len} samples in {num_batches} batches (batch_size={self.batch_size})...')
        ...
        for batch_idx in tqdm(range(num_batches), desc='Processing batches'):
            ...
            end_idx = min((batch_idx + 1) * self.batch_size, data_len)
            ...
            for i in range(start_idx, end_idx):
                ...
                GCNData.target_seq = torch.LongTensor([tok_smi])
                GCNData.__setitem__('c_size', torch.LongTensor([c_size]))
                batch_data_list.append(GCNData)
            
            # 应用pre_filter和pre_transform
            if self.pre_filter is not None:
                batch_data_list = [data for data in batch_data_list if self.pre_filter(data)]
            
            if self.pre_transform is not None:
                batch_data_list = [self.pre_transform(data) for data in batch_data_list]
        ...
        print('Data preparation Done!. Collating and saving to file...')
