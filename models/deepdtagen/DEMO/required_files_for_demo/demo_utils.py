...
def process_single_sample(smile, protein_seq):
    """直接处理单个样本，返回 PyTorch Geometric Data 对象，不依赖数据集类"""
    # 处理 SMILES 图
    c_size, features, edge_index, edge_feats = smile_to_graph(smile)
...
# def process_latent_a(smile, protein_seq):
#     # 生成唯一的哈希值作为文件名基础
#     file_hash = hashlib.md5(f"{smile}_{protein_seq}".encode()).hexdigest()
#     tokenizer_file = f'data/{file_hash}_tokenizer.pkl'
...
'''
# 修改 process_latent_a 和 process_latent 函数
def process_latent_a(smile, protein_seq):
    tokenizer_file = f'data/{smile}_tokenizer.pkl'
    tokenizer = Tokenizer(Tokenizer.gen_vocabs(smile))
    ...
    name = datetime.now().strftime("%Y%m%d%H%M%S")
    # 创建数据集对象但返回字典
    dataset = TestbedDataset(root='data', dataset=name, xd=np.asarray([smile]), xt=np.asarray([XT]),
                             smile_graph=smile_graph)
...
def process_latent(smile, protein_seq, affinity):
    ...
    name = datetime.now().strftime("%Y%m%d%H%M%S")
    # 创建数据集对象但返回字典
    dataset = TestbedDataset2(root='data', dataset=name, xd=np.asarray([smile]), xt=np.asarray([XT]), y=Y,
                              smile_graph=smile_graph)
...
class Tokenizer:
    ...
    def _reconstruct_data_list(self):
        """从保存的数据中重建数据列表"""
        try:
            # 如果已经有数据列表，直接返回
            if self.data_l:
            ...
            for i in range(len(self)):
                self.data_l.append(self[i])

            print(f"Reconstructed data list with {len(self.data_l)} items")
        except Exception as e:
            print(f"Error reconstructing data list: {str(e)}")
            # 作为最后手段，尝试使用 data.data
            if hasattr(self, 'data') and hasattr(self.data, 'data'):
                print("Falling back to self.data.data")
                self.data_l = self.data.data
            else:
                print("No fallback available - initializing empty list")
                self.data_l = []
    ...
    def __len__(self):
        # 添加保护性检查
        if not self.data_l:
            print("Warning: data_l is empty, attempting reconstruction")
            self._reconstruct_data_list()

            # 如果仍然为空，尝试使用 self.data
            if not self.data_l and hasattr(self, 'data'):
                try:
                    # 尝试从 self.data 获取长度
