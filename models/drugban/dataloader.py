...
class DTIDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if actual_node_feats.shape[0] > self.max_drug_nodes:
            next_index = (index + 1) % len(self.list_IDs)
...
class VStestDataset(data.Dataset):
    def __init__(self, list_IDs, df, Sequence, max_drug_nodes=500):
        self.valid_ids = []
        for ID in tqdm(list_IDs):
            row = df.loc[ID]
            if Chem.MolFromSmiles(row['SMILES']) is not None:
                self.valid_ids.append(ID)
        self.list_IDs = self.valid_ids
        self.df = df
        self.max_drug_nodes = max_drug_nodes
