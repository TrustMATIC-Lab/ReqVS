...
if __name__ == "__main__":
    ...
    # 优化：支持断点续传，跳过已完成的 Fold
    # 重新从 Fold 1 (index 0) 开始，确保完整性
    START_FOLD = 0 
    
    for i_fold in range(START_FOLD, K_Fold):
        # SEED = i_fold
        # random.seed(SEED)
        ...
        """ create model"""
        model = ColdstartCPI(unify_num=512,head_num=4,dataset = DATASET)
        ...
        # === 关键修复：显式释放内存，防止 Fold 2 OOM ===
        del train_dataset_load, valid_dataset_load, test_dataset_load
        del model
        import gc
        gc.collect()
        torch.cuda.empty_cache()
        print(f"Fold {i_fold + 1} finished. Memory cleared.")
        # ===============================================
